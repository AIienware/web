  ...
  var bradleyController = {
    ...
    _moveUtils: function() {
      ...
      if ($(window).width() > base.breakpoints.tablet) {
        if ($('.collapsed-nav').children('.wsite-search').length) {
          $('.wsite-search').insertBefore('.search-toggle');
        }
        if ($('.dummy-nav .wsite-menu-default').children('#member-login').length) {
          var member = $('#member-login').detach();
          member.insertBefore('.wsite-search');
        }
      } else {
       $('.wsite-search').prependTo('.collapsed-nav');
       var member = $('#member-login').detach();
       member.appendTo('.collapsed-nav');
      }
    },
